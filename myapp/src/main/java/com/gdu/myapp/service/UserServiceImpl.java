...
public class UserServiceImpl implements UserService {
  ...
  public void signin(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      UserDto user = userMapper.getUserByMap(params);  // select
      
      // 일치하는 회원 있음 (Sign In 성공)
      if(user != null) {
        //  접속 기록 ACCESS_HISTORY_T남기기
        userMapper.insertAccessHistory(params);
        // 회원 정보를 세션(브라우저 닫기 전까지 정보가 유지되는공간, 기본 30분 정보 유지)에 보관하기
        request.getSession().setAttribute("user", user); //${sessionScope.user}
        // Sign In 후 페이지 이동
