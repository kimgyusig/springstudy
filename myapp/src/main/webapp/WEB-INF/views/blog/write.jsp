      ...
      callbacks: {
        onImageUpload: (images)=>{
          ...
          for(let i = 0; i < images.length; i++) {
            let formData = new FormData();  // 코드로 form태그 만듬 <form>
            formData.append('image', images[i]); // <input type="file" name="image">
            fetch('${contextPath}/blog/summernote/imageUpload.do', { // 서버에 하나씩 저장
              method: 'POST',
              body: formData
            ...
            .then(resData=>{
              $('#contents').summernote('insertImage', '${contextPath}'+ resData.src); // 받아온걸 jsonsrc로 insertImage 속성으로저장
            })                                            // Map.of("src", "image url")  마직막에 <img src="image url"> 태그로 저장된다
          }
        }
      }
    })
