...
const fnGetBlogList = ()=> {
	...
	$.ajax({
		...
		data: 'page=' + page,  // url 에서 parameter 분리가능		
		// 응답
		dataType: 'json',
...
const fnScrollHandler = ()=> {
	...
	$(window).on('scroll', (evt)=>{
	  ...
	  if(timerId) { // timerId 가 undefined 이면 false, 아니면 true
		              // timerId 가 undefined 이면 setTimeout() 함수가 동작한 적 없음을 뜻함
...
const fnBlogDetail = ()=> {
	$(document).on('click', '.blog',(evt)=>{
		// <div class="blog"> 중 클릭 이벤트가 발생한 <div> : 이벤트대상
		...
		// 남이 작성한 브로그는 /update.do 요청 (조회수 증가가 있음)
